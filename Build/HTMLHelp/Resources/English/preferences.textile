...
The active set can be changed using the Macros menu.  If the "None"
 set is chosen, all macros are disabled.  Otherwise, the macros
 can be used by typing the key equivalent displayed in the menu
 or by selecting its name from the Macros menu.
...
Settings that control "kiosk mode", the Enter Full Screen command
 in the View menu.  You can use these preferences to create a
 more complete kiosk (by hiding the menu bar, for example).
...
If you want the menu bar to be completely unusable, disable this
 option.  Note that when "Use fast (non-system) mode" is unset,
 the system always makes the menu bar available.
...
If unset, the content region of a terminal window fills its whole
 ...
 a scroll bar).  Note that when "Use fast (non-system) mode" is
 unset, the system will keep the toolbar in Full Screen if it was
 visible outside of Full Screen, and the window title is hidden.
...
This option is only available when "Use fast (non-system) mode"
 is set, because MacTerm cannot customize the system's standard
 Full Screen mechanism.

h4. Method

h5. Use fast (non-system) mode

Suppresses the normal Full Screen mechanism of macOS and uses
 a custom MacTerm scheme instead.

The macOS mode has advantages and disadvantages; it is generally
 better than the custom scheme for small single screens such as
 laptops, and generally worse on desktops with multiple displays.
 The macOS method is more integrated with the system, allowing
 gestures and the ability to create more Full Screen windows than
 you have displays.  On the other hand, it adds slow animations
 and most of the Access Control preferences are not available
 because the system behavior cannot be changed by MacTerm.

Note that the system will automatically put a standard button in
 your terminal windows when the system mode is in effect.  Using
 View >> Customize Toolbar... you can remove the MacTerm toolbar
 item for Full Screen if you wish (its effect is the same).
