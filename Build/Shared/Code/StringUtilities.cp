...
to bugs if ÒconstnessÓ is casted away incorrectly.
...
and can lead to bugs if ÒconstnessÓ is casted away incorrectly.
...
graphics port, along with an acceptable ÒmaximumÓ
...
include an ellipsis (É) character.  The position
							 ...
							 StringUtilitiesTruncationMethod	inTruncationMethod)
{
	...
	if (result)
	{
		UInt16 const	kMaximumWidth = inAcceptableMaximumWidth - STATIC_CAST(CharWidth('É'), UInt16);
		...
		if (PLstrlen(inoutString) > 1) switch (inTruncationMethod)
		{
			case kStringUtilities_TruncateAtBeginning:
				{
					...
					StringUtilities_PInsert(inoutString, "\pÉ");
				}
				break;
			...
			default:
				...
				{
					inoutString[--(inoutString[0])] = 'É';
				}
				break;
		}
		else if (PLstrlen(inoutString) == 1) inoutString[1] = 'É';
	}
