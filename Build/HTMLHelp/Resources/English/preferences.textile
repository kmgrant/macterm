...
p(#abstract). This page is arranged in the same categories as those in the
...
You can hold down the Option key prior to starting a window
 resize or entering Full Screen mode, to temporarily switch this
 preference.
...
Many substitution sequences correspond directly to the codes
 used by control keys.  For this reason, MacTelnet offers an
...
Note that macros with substitutions can fail if your macro
 ...
 that MacTelnet generated for your macro.
...
fn1. To see the octal versions of all ASCII characters,
 ...
 MacTelnet window and runs "man ascii").  Also note that the button
...
For remote sessions, you can either enter a command yourself, or
 use the "Server Settings..." button to help you.  This button
 opens the Servers panel.  Note that edits in the Servers panel
 overwrite the command line field!  To learn more about the
 Servers panel, review "its documentation":windows.html#serverbrowser.
...
The Emacs text editor has its own commands for moving the cursor
 ...
 the next word with option-right-arrow; MacTelnet will cause this
...
Largely compatible with any system that wants a VT100, this
 ...
 MacTelnet is a very good VT102 emulator.

h5. VT220

This device has additional capabilities like special keyboard
 keys and 8-bit controls.  MacTelnet is a very good VT220
...
Mostly a superset of VT emulators, the XTerm terminal type also
 ...
 MacTelnet supports certain standard sequences such as cursor
 ...
 for now.  MacTelnet's XTerm support has improved over time.
...
In addition, note that TEK vector graphics is a feature of XTerm
 that MacTelnet enables separately; see "Graphics (TEK)":#sessiontek.
...
* Some applications rely on bugs in common emulators or the original devices, and won't work unless MacTelnet is also buggy.
