...
to a model if the listenerÕs type is inconsistent
...
You canÕt directly add callback functions to a listener
...
IMPORTANT:	A listener will fail to be installed unless its type is
			...
			Òlogical ORÓ model canÕt work if its callbacks return no
			values.
