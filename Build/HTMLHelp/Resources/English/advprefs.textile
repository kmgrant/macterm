...
(defbottom). |\2(desc). Full-screen terminals are altered to look more like real terminal screens.|
...
(defbottom). |\2(desc). The running process is instructed not to echo data coming from text input.|
(deftop). |(key). @data-send-paste-block-size-bytes@|(types). _integer_|
(defbottom). |\2(desc). "Large" pastes in throttled mode are split into chunks no larger than this number of bytes.|
(deftop). |(key). @data-send-paste-method@|(types). _string_: @normal@ or @throttled@|
(defbottom). |\2(desc). Pastes are sent to the session all at once or split into chunks.|
...
(defbottom). |\2(desc). Using the Return or Enter key sends this sequence (character codes in C-style octal) to the session.|
...
(defbottom). |\2(desc). File captures begin automatically when the session starts.|
