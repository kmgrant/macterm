...
/*!
This method can convert a string of bytes into data for an
IP address that is acceptable to the networking software.
The first valid part of the given byte array is used, and
the rest is ignored.  Only groups of 4 octets are allowed;
simpler forms of dot notation are not recognized by this
routine.  This method returns "true" only if the conversion
succeeds; if "false" is returned, the output IP address is
invalid.

For future expandability, this routine uses the structure
"InetAddress", so that eventually a parser that understands
IP version 6.0 addresses, and port numbers, can be written.
											 ...
											 InetAddress*	outIPAddressPtr)
{
	...
	// permit leading space
	for (charPtr = (inBuffer + 1); charPtr <= (inBuffer + inBufferLength); ++charPtr)
	{
		if (*charPtr != ' ') break;
	}
	
	// now convert
	for (; charPtr <= (inBuffer + inBufferLength); ++charPtr)
	{
		if (*charPtr == '.')
		{
			if (++dotCount > 3) done = true;
			if (!done)
			{
				address <<= 8;
				address |= byteValue;
				...
				((*charPtr) <= '9'))	// using "isdigit()" would prevent LiteCarbonLib linkage
		{
			byteValue *= 10;
			byteValue += (*charPtr - '0');
			if (byteValue > 255) done = true;
		}
		else if (*charPtr == ' ') break;
		else done = true;
	}
	
	if (!done)
	{
		// 3 dots would mean four numbers; is it valid?
		if (dotCount == 3)
		{
			address <<= 8;
			address |= byteValue;
			outIPAddressPtr->fAddressType = AF_INET;
			outIPAddressPtr->fPort = 0; // currently not parsed
			outIPAddressPtr->fHost = address;
			result = true;
		}
	}
	
	return result;
}// BufferToInternetAddress
