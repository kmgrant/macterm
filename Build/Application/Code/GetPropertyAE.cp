...
help handle a “get data” Apple Event.
...
kind of object - the “other” type would be
"inDesiredDataType".
...
this routine to help handle a “get data size”
Apple Event.
...
desired property and the class having the
property.  You generally use this routine to
help handle a “set data” Apple Event.
			...
			else if (inPropertyAccessMechanism == kPropertyAccessModify)
			{
				...
				if (canModifyProperty)
				{
					...
					if (result == noErr)
					{
						...
						if (result == noErr)
						{
							// set the length (it’s a Pascal string) and then set the title of the window
					 ...
					 AEDesc*		outObjectOfDesiredClass)
{
	...
	Console_BeginFunction();
	Console_WriteValueFourChars("AppleScript: “property” by property ID", inPropertyID);
			...
			case cMyWindow:
				{
					...
					{
						// return the window’s name as the specifier; not as good as an index, especially
