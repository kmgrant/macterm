...
(defbottom). |\2(desc). The "Force Quit" system command is available in Full Screen mode.  Always true unless using @kiosk-no-system-full-screen-mode@.|
(deftop). |(key). @kiosk-menu-bar-visible@|(types). _true or false_|
(defbottom). |\2(desc). The menu bar can be reached in Full Screen mode by moving the mouse to the top of the screen.  Always true unless using @kiosk-no-system-full-screen-mode@.|
(deftop). |(key). @kiosk-no-system-full-screen-mode@|(types). _true or false_|
(defbottom). |\2(desc). Full-screen terminals do not use the contemporary OS X animated mode.|
(deftop). |(key). @kiosk-off-switch-visible@|(types). _true or false_|
(defbottom). |\2(desc). A floating window is visible to turn off Full Screen mode.|
...
(defbottom). |\2(desc). The window title and toolbar are visible in Full Screen mode.  Always true for the toolbar unless using @kiosk-no-system-full-screen-mode@.|
...
(deftop). |(key). @no-update-warning@|(types). _true or false_|
(defbottom). |\2(desc). Messages recommending new versions of the program are not displayed.|
...
table(deftable).
(deftop). |(key). @macro-NN-action@, where NN is 01 to 12|(types). _string_: @text@, @verbatim@, @handle URL@, @select matching window@ or @new window with command@|
(defbottom). |\2(desc). How to interpret the content string.  If @text@, then escapes such as @\012@ are expanded; if @verbatim@, escapes are ignored.|
(deftop). |(key). @macro-NN-contents-string@, where NN is 01 to 12|(types). _string_|
(defbottom). |\2(desc). Text, whose meaning depends on the action: something to send (interpreted or verbatim), a URL to open, or a command line to run (without interpretation).|
...
(defbottom). |\2(desc). This Format collection is used by default when the session starts.|
...
(deftop). |(key). @terminal-capture-directory-alias@|(types). _data_: alias information|
(defbottom). |\2(desc). Automatically-captured file is saved to this location.  The data is a system "alias record" that contains enough information to resolve the directory location later even if (say) the user has since renamed it.  See @terminal-capture-auto-start@.|
