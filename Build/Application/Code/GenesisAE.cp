...
The "Terminology.h" file, also used for MacTelnetÕs
Apple Event Terminology Extension resource, contains
constants for all of MacTelnetÕs class types.
...
NOTE:	Classes are automatically evolved; unless
		there is no way to derive a child classÕ data
...
the internal data for an objectÕs container.
											 ...
											 ObjectClassesAE_TokenPtr   outTokenDataPtr)
{
	...
	else
	{
		...
		// the class owning the requested property is provided explicitly (coercion probably isnÕt needed)
...
Òis alsoÓ an instance of the second class.  For example,
...
can be used to ÒevolveÓ an object token.  This is done
			 ...
			 DescType*					outFinalClassTypeFromTerminologyOrNull)
{
	...
	default:
		// the specified token canÕt be evolved any further
		break;
	}
