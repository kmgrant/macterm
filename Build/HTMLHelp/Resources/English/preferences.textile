...
If a panel has one or more columns of check boxes on the side, it
 means that you can delete any "local" values for that setting
 and force the collection to always inherit the default value.
 When the Default is changed, inheriting collections will always
 use the latest Default value.

For example, you may decide that you want every terminal window
 to use the same font and size but allow different Format options
 to have different colors.  To have the font kept in sync
 automatically, simply set the font and size in the Default Format
 and ensure that the appropriate check boxes are set in all other
 Format collections to use that Default.

When there is more than one check box side-by-side, each box
 corresponds to a different setting in the same row.  For example,
 if you see two color boxes side-by-side, each will have its own
 check box to control inheritance of one color.

The "Use Default" check boxes are disabled once they are checked.
 Enter a different value to make the "Use Default" option
 available again; for example, if the check box controls a color,
 click the color box to select a new color and the check box will
 be enabled.
