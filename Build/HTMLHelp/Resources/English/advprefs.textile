...
(defbottom). |(desc)\2. The window title and toolbar are visible in Full Screen mode.|
...
(defbottom). |(desc)\2. The character(s) used to terminate lines in generated text files.|
...
(defbottom). |(desc)\2. A thin vertical line is rendered after this column of text; 1 is first column, 0 turns off the feature.|
...
(defbottom). |(desc)\2. When tabs are enabled, this side of the window should be used.|

h3(#macros). Macro Collections

table(deftable).
(deftop). |(key). @macro-NN-action@, where NN is 01 to 12|(types). _string_: @text@, @verbatim@, @handle URL@, or @new window with command@|
(defbottom). |(desc)\2. How to interpret the content string.  If @text@, then escapes such as @\012@ are expanded; if @verbatim@, escapes are ignored.|
(deftop). |(key). @macro-NN-contents-string@, where NN is 01 to 12|(types). _string_|
(defbottom). |(desc)\2. Text, whose meaning depends on the action: something to send (interpreted or verbatim), a URL to open, or a command line to run (without interpretation).|
(deftop). |(key). @macro-NN-key@, where NN is 01 to 12|(types). _string_: a "key name":#specialvalues|
(defbottom). |(desc)\2. Base key that activates this macro, when combined with the modifier keys.|
(deftop). |(key). @macro-NN-modifiers@, where NN is 01 to 12|(types). _array of string_: zero or more "modifier keys":#specialvalues|
(defbottom). |(desc)\2. Modifier keys that must be held down before the base key is interpreted as a macro request.|
(deftop). |(key). @macro-NN-name-string@, where NN is 01 to 12|(types). _string_|
(defbottom). |(desc)\2. Name to use in menus and other interface elements, when representing this macro.|
(deftop). |(key). @macro-order@|(types). _array of string_: values from @macro-01@ to @macro-12@|
(defbottom). |(desc)\2. Order in which macros should be displayed in user interface elements.|
...
(defbottom). |(desc)\2. The top-left corner of the window on its display, and the frame size.|
...
(defbottom). |(desc)\2. The boundaries of the display the window was on when the frame was saved.|
(deftop). |(key). @window-NN-name-string@, where NN is 01 to 10|(types). _string_|
(defbottom). |(desc)\2. Name to use in the window title bar and other interface elements, when representing this window.|
(deftop). |(key). @window-NN-session-built-in@, where NN is 01 to 10|(types). _string_: @default@, @dialog@, @log-in shell@ or @shell@|
(defbottom). |(desc)\2. This type of session is started when the window opens.  Ignored if there is a @window-NN-session-favorite@.|
(deftop). |(key). @window-NN-session-favorite@, where NN is 01 to 10|(types). _string_|
(defbottom). |(desc)\2. The command in this Session collection is run when the window opens.  Overrides @window-NN-session-built-in@.|
...
(deftop). |(key). @data-send-paste-method@|(types). _string_: @normal@ or @throttled@|
...
(defbottom). |(desc)\2. Prevents the multi-line Paste warning from appearing, and does not join pasted lines together.|
...
(deftop). |(key). @key-map-new-line@|(types). _string_: @\015\000@ or @\015\012@| 
...
(deftop). |(key). @server-protocol@|(types). _string_: @ssh-1@, @ssh-2@, @sftp@, @ftp@ or @telnet@| 
...
(defbottom). |(desc)\2. Image file to serve as the background when rendering almost all text.|
