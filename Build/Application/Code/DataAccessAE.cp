...
/*!
This routine indirectly handles all “get data”
Apple Events.

When a script asks for data, AppleScript will
figure out what is being said (for example,
"window 1" means “a window referenced by
absolute index”) and invoke an object accessor
...
handle it.  Most “data” is attached to a high
			 ...
			 Boolean*   outUsedDesiredDataType)
{
	...
	Console_WriteLine("AppleScript: “get data” event");
	Console_WriteValueFourChars("desired type", inGetDataOfThisType);
	
	if (inFromWhichObject->descriptorType == typeObjectSpecifier)
	{
		// all that’s known is that this is an object; resolve it into a known object type
...
/*!
This routine indirectly handles all “get data size”
Apple Events.
				 ...
				 Boolean*   outUsedDesiredDataType)
{
	...
	Console_WriteLine("AppleScript: “get data size” event");
	Console_WriteValueFourChars("desired type", inGetDataOfThisType);
...
/*!
This routine indirectly handles all “set data”
Apple Events.
			 ...
			 AEDesc*			inDataDescriptor)
{
	...
	Console_WriteLine("AppleScript: “set data” event");
