...
Note that MacTelnet also supports saving sessions to files with
File >> Save As, though those files do not save as many details
as Preferences do.
...
Use Window >> Show Macro Editor to display an older interface
for editing macros, currently the most reliable.  (Eventually the
Preferences window will be the only editing method.)

Macros in the Preferences window can be edited directly in their
list.
...
terminal window.  Custom and Favorite sessions can associate a
Terminal Favorite with them, ensuring that these important settings
are always made for you.
