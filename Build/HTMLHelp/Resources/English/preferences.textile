...
h5(#macrosmenu). Display a macros menu

Show a Macros menu in the menu bar, providing direct access to
the current macros (as selected from the Map menu and defined in
the Preferences window).  Also useful for seeing at a glance the
names and key equivalents of your current macros.  And, if you
invoke keys that trigger macros, the Macros menu title will
flash.
...
So far in MacTelnet 4.0 beta, these preferences are not fully
functional, but the intent is to offer very flexible window
management.
...
use the "Connect to Server..." button to help you.  This button
