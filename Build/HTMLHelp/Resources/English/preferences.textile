...
Local Echo is useful for slow connections, because it displays
 ...
 response.  (The floating Command Line in MacTerm gives a
 similar benefit; see the Window menu.)

There are downsides to echoing, that basically boil down to how
 well the session and MacTerm agree.  When half-duplex, the
...
MacTerm allows you to locally buffer Paste on a per-session
 basis.  This means that MacTerm does not send all text at once,
