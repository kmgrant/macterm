											 ...
											 SessionDescription_Ref		inSessionDescription)
{
	...
	// see if this is a command line session...
	{
		...
		if (kSessionDescription_ResultCodeSuccess == dataAccessError)
		{
			// okay, that data is in the file; this means it describes a ÒlocalÓ session...
		...
		else
		{
			...
			if (kSessionDescription_ResultCodeSuccess == dataAccessError)
			{
				// okay, host name data is in the file; this means it describes a ÒremoteÓ session...
				// UNIMPLEMENTED
				// (NOTE: currently on Mac OS X, all ÒremoteÓ sessions are effectively local commands...)
			}
			else
			{
				// ???
...
regardless of state (i.e. it doesnÕt have to be
connected yet).
...
userÕs new session was created.
...
manipulate those sessions (for example, hiding a sessionÕs
window).
...
WARNING:	It is not safe to use this routine with
			...
			windows, sessions, or other Òfirst classÓ
			objects.
...
even if they arenÕt connected yet).
...
Invoked by Mac OS X whenever a custom Òdata is available
for processingÓ event is posted (presumably by a
...
Invoked by Mac OS X whenever a custom Òset session
stateÓ event is posted (presumably by a preemptive
						 ...
						 TerminalWindowRef		inTerminalWindow)
{
	...
	// ensure that changes to the specified session invoke the Session FactoryÕs notifier
...
stopTrackingSession		(SessionRef		inSession)
{
	...
	// ensure that changes to the specified session no longer invoke the Session FactoryÕs notifier
